...
//        Time a sequential implementation for reference
        ...
        for (int run_no = 0; run_no < NUM_RUNS; run_no++) {
            ...
            printf("  expoly with FPE2 and superacc    = %.16g  poly = %.16g\n", expoly_fpe2.sum, expoly_fpe2.mts);
            printf("  expoly with FPE4 and superacc    = %.16g  poly = %.16g\n", expoly_fpe4.sum, expoly_fpe2.mts);
            printf("  expoly with FPE4ee and superacc  = %.16g  poly = %.16g\n", expoly_fpe4ee.sum,
                   expoly_fpe4ee.mts);
            printf("  expoly with FPE6ee and superacc  = %.16g  poly = %.16g\n", expoly_fpe6ee.sum,
                   expoly_fpe6ee.mts);
            printf("  expoly with FPE8ee and superacc  = %.16g  poly = %.16g\n", expoly_fpe8ee.sum,
                   expoly_fpe8ee.mts);
