 ...
 */
#ifndef EXMAXSUM_FPE_HPP_
#define EXMAXSUM_FPE_HPP_
...
bool horizontal_or(double a){
    return a != 0;
}

/**
 * \struct FPExpansionVect_Max
 * \ingroup ExSUM
...
struct FPExpansionVect_Max
{
     ...
     */
    FPExpansionVect_Max(Superaccumulator& sa);
    ...
    /**
     * This function records max value when adding x to the floating-point expansion
     * \param x input value
     */
    void Max(T x);
...
FPExpansionVect_Max<T,N,TRAITS>::FPExpansionVect_Max(Superaccumulator & sa) :
        superacc(sa),
        victim(0)
{
    std::fill(a, a + N, 0);
...
void FPExpansionVect_Max<T,N,TRAITS>::Accumulate(T x)
{
    // Experimental
...
template<typename T, int N, typename TRAITS> UNROLL_ATTRIBUTE
void FPExpansionVect_Max<T,N,TRAITS>::Max(T x)
{
    T s;
    T val = 0;
    for(unsigned int i = 0; i != N; ++i) {
...
template<typename T, int N, typename TRAITS>
T FPExpansionVect_Max<T,N,TRAITS>::twosum(T a, T b, T & s)
{
#if INSTRSET > 7                       // AVX2 and later
...
template<typename T, int N, typename TRAITS>
void FPExpansionVect_Max<T,N,TRAITS>::Swap(T & x1, T & x2)
{
...
template<typename T, int N, typename TRAITS> UNROLL_ATTRIBUTE
void FPExpansionVect_Max<T,N,TRAITS>::Insert(T & x)
{
    if(TRAITS::Sort) {
...
template<typename T, int N, typename TRAITS> UNROLL_ATTRIBUTE
void FPExpansionVect_Max<T,N,TRAITS>::Insert(T & x1, T & x2)
{
    if(TRAITS::Sort) {
        // x1 <= a[0]
        // x2 <= a[1]
...
void FPExpansionVect_Max<T,N,TRAITS>::Accumulate(T x1, T x2)
{
...
template<typename T, int N, typename TRAITS>
void FPExpansionVect_Max<T,N,TRAITS>::Flush()
{
    for(unsigned int i = 0; i != N; ++i)
    {
        FlushVector(a[i]);
...
void FPExpansionVect_Max<T,N,TRAITS>::FlushVector(T x) const
{
    superacc.Accumulate(x);
}

template<typename T, int N, typename TRAITS>
void FPExpansionVect_Max<T,N,TRAITS>::Dump() const
{
    for(unsigned int i = 0; i != N; ++i)
    {
        DumpVector(a[i]);
...
template<typename T, int N, typename TRAITS>
void FPExpansionVect_Max<T,N,TRAITS>::DumpVector(T x) const
{
